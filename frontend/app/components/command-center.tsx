  ...
  return <main className="command-center">
    ...
    <section className="command-grid">
      ...
      <article className="command-panel twin-panel">
        <div className="command-panel-head"><div><span>SPATIAL DIGITAL TWIN</span><h2>Network topology</h2></div><div className="arm-switch" role="group" aria-label="Network strategy"><button type="button" className={arm === "baseline" ? "active danger" : ""} onClick={() => setArm("baseline")} aria-pressed={arm === "baseline"}>Baseline</button><button type="button" className={arm === "vidyut" ? "active" : ""} onClick={() => setArm("vidyut")} aria-pressed={arm === "vidyut"}>Vidyut</button></div></div>
    ...
    <section className="command-timeline">
      <div className="timeline-controls"><button type="button" onClick={() => { if (tick >= recording.ticks.length - 1) setTick(0); setPlaying((value) => !value); }} aria-label={playing ? "Pause replay" : "Play replay"} aria-pressed={playing}>{playing ? "Ⅱ" : "▶"}</button><button type="button" onClick={() => { setPlaying(false); setTick(Math.max(0, tick - 1)); }} aria-label="Previous interval">←</button><button type="button" onClick={() => { setPlaying(false); setTick(Math.min(recording.ticks.length - 1, tick + 1)); }} aria-label="Next interval">→</button></div>
